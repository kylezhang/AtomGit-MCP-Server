...
export class OrganizationService extends BaseService {
    ...
    async getUserOrganizations(username, page = 1, perPage = 30) {
        const response = await this.client.get(`/api/v5/users/${username}/orgs`, {
            params: { page, per_page: perPage }
        });
        return response.data;
    }
    async getCurrentUserOrganizations(page = 1, perPage = 30) {
        const response = await this.client.get('/api/v5/users/orgs', {
            params: { page, per_page: perPage }
        });
        return response.data;
    }
    async getOrganizationMember(org, username) {
    ...
    async updateOrganization(org, updateData) {
    ...
    async getCurrentUserOrganizationMembership(org) {
    ...
    async getOrganizationMembers(org, page = 1, perPage = 30) {
        const response = await this.client.get(`/api/v5/orgs/${org}/members`, {
            params: { page, per_page: perPage }
        });
        return response.data;
    }
    async getEnterpriseMembers(enterprise, page = 1, perPage = 30) {
