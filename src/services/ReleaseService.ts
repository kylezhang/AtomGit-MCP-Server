...
export class ReleaseService extends BaseService {
  ...
  async updateRelease(owner: string, repo: string, tag: string, releaseData: UpdateReleaseRequest): Promise<Release> {
    const response = await this.client.patch(`/api/v5/repos/${owner}/${repo}/releases/${this.encodeName(tag)}`, releaseData);
    return response.data;
  }

  async getReleaseUploadUrl(owner: string, repo: string, tag: string, fileName?: string): Promise<any> {
    const response = await this.client.get(`/api/v5/repos/${owner}/${repo}/releases/${this.encodeName(tag)}/upload_url`, {
  ...
  async getRelease(owner: string, repo: string, tag: string, tempDownloadUrl?: string | boolean): Promise<Release> {
    const response = await this.client.get(`/api/v5/repos/${owner}/${repo}/releases/${this.encodeName(tag)}`, {
  ...
  async getReleaseByTag(owner: string, repo: string, tag: string): Promise<Release> {
    const response = await this.client.get(`/api/v5/repos/${owner}/${repo}/releases/tags/${this.encodeName(tag)}`);
    return response.data;
  }
  ...
  async downloadReleaseAsset(owner: string, repo: string, tag: string, fileName: string): Promise<any> {
    const response = await this.client.get(`/api/v5/repos/${owner}/${repo}/releases/${this.encodeName(tag)}/attach_files/${this.encodePath(fileName)}/download`);
    return response.data;
  }

  async deleteReleaseAttachment(owner: string, repo: string, tag: string, attachFileId: string | number): Promise<void> {
    await this.client.delete(`/api/v5/repos/${owner}/${repo}/releases/${this.encodeName(tag)}/attach_files/${attachFileId}`);
  }
}
